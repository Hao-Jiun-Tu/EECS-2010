...
extern __constant__ int  mask[MASK_N][MASK_X][MASK_Y] = { 
    {{ -1, -4, -6, -4, -1},
     { -2, -8,-12, -8, -2},
     {  0,  0,  0,  0,  0}, 
     {  2,  8, 12,  8,  2}, 
...
__global__ void sobel(unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels){
    ...
    for( y = blockIdx.x; y < height; y += gridDim.x){
...
int main(int argc, char** argv) {
    ...
    unsigned char* dev_s;
    unsigned char* dev_t;
    cudaMalloc(&dev_s, sizeof(unsigned char) * height * width * channels);
    cudaMalloc(&dev_t, sizeof(unsigned char) * height * width * channels);
    cudaMemcpy(dev_s, host_s, sizeof(unsigned char) * height * width * channels, cudaMemcpyHostToDevice);
    sobel<<<1024, 1024>>>(dev_s, dev_t, height, width, channels);
